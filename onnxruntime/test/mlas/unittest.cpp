...
private:
        ...
        )
    {
        ...
        for (size_t n = 0; n < N; n++) {
            ...
            OutputReference[n] = QLinearMulScalar(InputA[n], ScaleA, ZeroPointA, InputB[IsScalarB ? 0 : n], ScaleB, ZeroPointB, ScaleC, ZeroPointC);
        }

        MlasQLinearMul(InputA, ScaleA, ZeroPointA, InputB, ScaleB, ZeroPointB, ScaleC, ZeroPointC, OutputC, N, IsScalarB);
        ...
        for (size_t a = 0; a < _countof(zero_points); a++) {
            for (size_t b = 0; b < _countof(zero_points); b++) {
                for (size_t c = 0; c < _countof(zero_points); c++) {
                    for (size_t n = 1; n < 128; n++) {
                        // u8, vector + vector
                        Test<uint8_t, false>(n, 10.f, zero_points[a], 10.f, zero_points[b], 20.f, zero_points[c]);

                        // u8, vector + scalar
                        Test<uint8_t, true>(n, 10.f, zero_points[a], 10.f, zero_points[b], 20.f, zero_points[b]);

                        // s8, vector + vector
                        Test<int8_t, false>(n, 10.f, s_zero_points[a], 10.f, s_zero_points[b], 20.f, s_zero_points[c]);

                        // s8, vector + scalar
                        Test<int8_t, true>(n, 10.f, s_zero_points[a], 10.f, s_zero_points[b], 20.f, s_zero_points[b]);
                    }
                }
            }
        }
...
class MlasQLinearMulTest : public MlasTestBase
{
private:
    MatrixGuardBuffer<uint8_t> BufferInputA;
    MatrixGuardBuffer<uint8_t> BufferInputB;
